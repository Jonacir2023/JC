...
### Bug encontrado na migração (v20.1)
A migração automática de v3 → v4 extraía o nome da obra de `obra_atual` (que era vazia na primeira abertura), criando uma chave `diario_obras_v4_state_` sem nome da obra. Isso fazia com que:

1. Dados antigos de Rio Tanque eram migrados para chave genérica (sem obra)
2. Quando o usuário depois mudava para Suzano e abria de novo, o app carregava a chave **vazia** (com Rio Tanque)
3. Em vez de reconhecer que aquela chave tinha dados de outra obra

**Correção v20.1:**
- `loadState()` e `loadHistory()` agora **extraem o nome da obra dos dados antigos** durante migração
- Se os dados antigos de v3 têm `obra.nome = "Rio Tanque"`, a chave fica como `diario_obras_v4_state_rio_tanque`
- Assim, Rio Tanque e Suzano ficam isolados corretamente na primeira abertura
